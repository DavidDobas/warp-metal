...
# Python files that differ from stock but must not, or need not, be overlaid: the reason, and the text
# every differing line has to contain for that reason to be true (None: the file is never used at run time).
NOT_OVERLAID = {
    "__init__.py": (
        "only adds the is_metal_available export, which the import hook provides",
        "is_metal_available",
    ),
    "__init__.pyi": ("type stub; only adds is_metal_available", "is_metal_available"),
    "config.py": ("differs only in the recorded git commit hash", "_git_commit_hash"),
    "_src/build_dll.py": ("only used to build the native library", None),
}

# Both derive ``warp_home`` from their own location, which is what makes the overlay load
# ``warp_metal/bin`` and compile kernels against ``warp_metal/native``. They must be overlaid together.
PATH_ANCHORS = ("build", "context")
...
def stock_metadata(stock):
    ...
    commit = re.search(
        r'^_git_commit_hash: str \| None = "([0-9a-f]{40})"', config, re.M
    )
    if not version:
...
def unexplained_differences(fork, stock, rel, marker):
    """Differing lines of ``warp/<rel>`` that do not contain ``marker``."""
    import difflib  # noqa: PLC0415

    a = read_normalized(os.path.join(stock, "warp", rel)).decode().splitlines()
    b = read_normalized(os.path.join(fork, "warp", rel)).decode().splitlines()
    changed = [line[1:] for line in difflib.ndiff(a, b) if line[:1] in "+-"]
    return [line for line in changed if line.strip() and marker not in line]
...
def differing_python_files(fork, stock):
    ...
    for base, dirs, files in os.walk(fork_pkg):
        dirs[:] = [
            d
            for d in dirs
            if d not in ("__pycache__", "bin", "native", "tests", "examples")
        ]
        for name in files:
            ...
            if not os.path.exists(stock_file) or read_normalized(
                os.path.join(base, name)
            ) != read_normalized(stock_file):
...
def main():
    ...
    ap.add_argument(
        "--stock",
        required=True,
        help="directory with the unpacked stock warp-lang wheel",
    )
    ap.add_argument(
        "--revision",
        default=None,
        help="two-digit revision appended to the Warp version",
    )
    ap.add_argument(
        "--allow-dirty",
        action="store_true",
        help="accept uncommitted changes in the fork",
    )
    args = ap.parse_args()
    ...
    if fork_version != warp_version:
        sys.exit(
            f"error: the fork is at version {fork_version}, the stock wheel is {warp_version}"
        )
    if stock_commit is None:
        ...
        print(
            f"fork contains upstream {stock_commit[:12]} plus {ahead} first-parent commits"
        )
    commit = run(["git", "rev-parse", "HEAD"], fork)
    if (
        run(["git", "status", "--porcelain", "--untracked-files=no"], fork)
        and not args.allow_dirty
    ):
        sys.exit(
            "error: the fork checkout has uncommitted changes (pass --allow-dirty to accept them)"
        )
    ...
    for rel in differing_python_files(fork, stock):
        if rel in NOT_OVERLAID:
            reason, marker = NOT_OVERLAID[rel]
            if marker is not None:
                unexplained = unexplained_differences(fork, stock, rel, marker)
                if unexplained:
                    sys.exit(
                        f"error: warp/{rel} is not overlaid because it '{reason}', but it also differs in:\n  "
                        + "\n  ".join(unexplained[:10])
                    )
            print(f"not overlaid: warp/{rel} ({reason})")
    ...
    if not modules:
        sys.exit(
            "error: no module differs from stock; is --stock really the stock wheel?"
        )
    for anchor in PATH_ANCHORS:
        if (
            anchor not in modules
        ):  # identical to stock today: still has to come from the overlay
            modules.append(anchor)
            print(
                f"overlaid although identical to stock: warp/_src/{anchor}.py (locates bin/ and native/)"
            )
    modules.sort()
    ...
    for name in GENERATED:
        ...
        shutil.rmtree(path) if os.path.isdir(path) else os.path.exists(
            path
        ) and os.remove(path)
    ...
    for name in sorted(os.listdir(native_src)):
        ...
        if os.path.isfile(path) and (
            name.endswith((".h", ".exports")) or name == "crt.cpp"
        ):
            shutil.copy2(path, native_dst)
    for sub in NATIVE_SUBDIRS:
        shutil.copytree(
            os.path.join(native_src, sub),
            os.path.join(native_dst, sub),
            ignore=shutil.ignore_patterns("*.o", "*.cu"),
        )
    ...
    if not os.path.exists(library):
        sys.exit(
            "error: warp/bin/libwarp.dylib is missing; run `uv run build_lib.py` in the fork"
        )
    newest_source = max(
        os.path.getmtime(os.path.join(native_src, n))
        for n in os.listdir(native_src)
        if n.endswith((".cpp", ".mm", ".h"))
    )
    if os.path.getmtime(library) < newest_source:
        sys.exit(
            "error: warp/bin/libwarp.dylib is older than the native sources; rebuild it in the fork"
        )
    os.makedirs(os.path.join(PKG, "bin"))
    shutil.copy2(library, os.path.join(PKG, "bin"))
    # the build leaves its absolute output path as the install name; do not ship a local path
    subprocess.check_call(
        [
            "install_name_tool",
            "-id",
            "@rpath/libwarp.dylib",
            os.path.join(PKG, "bin", "libwarp.dylib"),
        ],
        stderr=subprocess.DEVNULL,
    )
    # editing the library invalidates its signature, and arm64 macOS refuses to load unsigned code
    subprocess.check_call(
        [
            "codesign",
            "--force",
    ...
    if args.revision is not None:
        ...
        text = re.sub(
            r'(?m)^version = "[^"]+"',
            f'version = "{package_version(warp_version, args.revision)}"',
            text,
        )
    open(pyproject, "w").write(text)
    print(
        f"generated from fork commit {commit[:12]} for warp-lang {warp_version}: overlays {', '.join(modules)}"
    )
...
def package_version(warp_version, revision):
    ...
    return (
        warp_version + revision
        if ".dev" in warp_version
        else f"{warp_version}.{revision}"
    )
