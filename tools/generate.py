...
def stock_metadata(stock):
    ...
    version = re.search(r'^version: str = "([^"]+)"', config, re.MULTILINE)
    commit = re.search(r'^_git_commit_hash: str \| None = "([0-9a-f]{40})"', config, re.MULTILINE)
    if not version:
...
def main():
    ...
    for anchor in PATH_ANCHORS:
        if anchor not in modules:  # identical to stock today: still has to come from the overlay
            modules.append(anchor)
            print(f"overlaid although identical to stock: warp/_src/{anchor}.py (locates bin/ and native/)")
    modules.sort()
    ...
    with open(os.path.join(overlay, "__init__.py"), "w"):
        pass
