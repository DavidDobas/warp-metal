...
/*
 * Please first read the randf comment. randf returns values uniformly distributed in the range [0.f, 1.f - 2.^-24] in
 * equal intervals of size 2.^-24. randn computes sqrt(-2.f * log(x)). For this to return a real value, log(x) < 0.f (we
 * exclude 0.f as a precaution) and therefore x < 1.f. For it to be finite, x > 0.f. So x must be in (0.f, 1.f). We
 * define RANDN_EPSILON to be 2^-24 truncated to 5.96e-8f and add it to the range of randf, giving the domain
 * [RANDN_EPSILON, 1.f - 2.^-24 + RAND_EPSILON] which satisfies the requirement that x is in (0.f, 1.f).
 */
...
inline CUDA_CALLABLE uint32 rand_init(int seed, int offset)
{
...
/*
 * We want to ensure randf adheres to a uniform distribution over [0,1). The set of all possible float32 (IEEE 754
 * standard) values is not uniformly distributed however. On the other hand, for a given sign and exponent, the mantissa
 * of the float32 representation is uniformly distributed. Fixing an exponent of -1, we can craft a uniform distribution
 * using the sign bit and 23-bit mantissa that spans the domain [0, 1) in 2^24 equal intervals. We can map 2^24 unique
 * unsigned integers to these 2^24 intervals, so if our random number generator returns values in the range [0, 2^24)
 * without bias, we can ensure that our float distribution in the range [0, 1) is also without bias. Our random number
 * generator returns values in the range [0, 2^32), so we bit shift a random unsigned int 8 places, and then make the
...
inline CUDA_CALLABLE float random_loggam(float x)
{
    ...
    const float a[10]
        = { 8.333333333333333e-02f, -2.777777777777778e-03f, 7.936507936507937e-04f, -5.952380952380952e-04f,
            8.417508417508418e-04f, -1.917526917526918e-03f, 6.410256410256410e-03f, -2.955065359477124e-02f,
