...
struct UintKeyToUint {
    ...
    inline CUDA_CALLABLE uint32_t max_possible_key_value() { return 0xFFFFFFFF; }
};
...
struct IntKeyToUint {
    ...
    inline CUDA_CALLABLE int max_possible_key_value() { return 2147483647; }
};
...
struct FloatKeyToUint {
    ...
    inline CUDA_CALLABLE float max_possible_key_value() { return FLT_MAX; }
};

struct Uint64KeyToUint {
    inline CUDA_CALLABLE uint64_t convert(uint64_t value) { return value; }

    inline CUDA_CALLABLE uint64_t max_possible_key_value() { return 0xFFFFFFFFFFFFFFFFULL; }
};
...
struct Int64KeyToUint {
    ...
    inline CUDA_CALLABLE int64_t max_possible_key_value() { return 9223372036854775807LL; }
};


constexpr inline CUDA_CALLABLE bool is_power_of_two(int x) { return (x & (x - 1)) == 0; }
...
inline CUDA_CALLABLE void bitonic_sort_single_stage_full_thread_block(
    int k,
...
#pragma unroll
    for (int loop_id = 0; loop_id < num_loops; ++loop_id) {
        ...
        if (thread_id2 < length) {
            ...
            swap[loop_id] = (((thread_id2 & stride) != 0 ? key_register[loop_id] > s_key[loop_id]
                                                         : key_register[loop_id] < s_key[loop_id]))
                ^ ((thread_id2 & k) == 0);
        }
    }
...
bitonic_sort_single_warp(int thread_id, K* keys_input, V* values_input, int num_elements_to_sort)
{
...
inline CUDA_CALLABLE void
bitonic_sort_pow2_length(unsigned int thread_id, K* key_sh_mem, V* val_sh_mem, int length, K key_max_possible_value)
{
...
#pragma unroll
                ...
                bitonic_sort_single_stage_full_thread_block<K, V, num_loops>(
                    k, thread_id, (unsigned int)stride, key_sh_mem, val_sh_mem, length, key_max_possible_value, key, val
                );
            }
        }
    }
...
bitonic_sort_thread_block_shared_mem(int thread_id, K* keys_input, V* values_input, int num_elements_to_sort)
{
    if constexpr (max_num_elements < WP_TILE_BITONIC_GROUP_SIZE) {
        ...
        bitonic_sort_pow2_length<shared_mem_count, K, V>(
            (unsigned int)thread_id, keys_shared_mem, values_shared_mem, shared_mem_count, key_max_possible_value
        );
...
inline CUDA_CALLABLE void
bitonic_sort_thread_block_shared_mem(int thread_id, int* keys_input, V* values_input, int num_elements_to_sort)
{
...
template <int max_num_elements, typename V>
inline CUDA_CALLABLE void
bitonic_sort_thread_block_shared_mem(int thread_id, unsigned int* keys_input, V* values_input, int num_elements_to_sort)
{
...
template <int max_num_elements, typename V>
inline CUDA_CALLABLE void
bitonic_sort_thread_block_shared_mem(int thread_id, float* keys_input, V* values_input, int num_elements_to_sort)
{
...
template <int max_num_elements, typename V>
inline CUDA_CALLABLE void
bitonic_sort_thread_block_shared_mem(int thread_id, int64_t* keys_input, V* values_input, int num_elements_to_sort)
{
...
template <int max_num_elements, typename V>
inline CUDA_CALLABLE void
bitonic_sort_thread_block_shared_mem(int thread_id, uint64_t* keys_input, V* values_input, int num_elements_to_sort)
{
...
bitonic_sort_thread_block_direct(int thread_id, K* keys_input, V* values_input, int num_elements_to_sort)
{
    if constexpr (max_num_elements < WP_TILE_BITONIC_GROUP_SIZE) {
        ...
        bitonic_sort_pow2_length<max_num_elements, K, V>(
            (unsigned int)thread_id, keys_input, values_input, num_elements_to_sort, key_max_possible_value
        );
    }
}
...
template <int max_num_elements, typename V>
inline CUDA_CALLABLE void
bitonic_sort_thread_block_direct(int thread_id, int* keys_input, V* values_input, int num_elements_to_sort)
{
...
template <int max_num_elements, typename V>
inline CUDA_CALLABLE void
bitonic_sort_thread_block_direct(int thread_id, unsigned int* keys_input, V* values_input, int num_elements_to_sort)
{
...
template <int max_num_elements, typename V>
inline CUDA_CALLABLE void
bitonic_sort_thread_block_direct(int thread_id, float* keys_input, V* values_input, int num_elements_to_sort)
{
...
template <int max_num_elements, typename V>
inline CUDA_CALLABLE void
bitonic_sort_thread_block_direct(int thread_id, int64_t* keys_input, V* values_input, int num_elements_to_sort)
{
...
template <int max_num_elements, typename V>
inline CUDA_CALLABLE void
bitonic_sort_thread_block_direct(int thread_id, uint64_t* keys_input, V* values_input, int num_elements_to_sort)
{
...
template <typename T> inline CUDA_CALLABLE T warp_scan_exclusive(int lane, T value)
{
...
inline CUDA_CALLABLE void radix_sort_thread_block_core(
    int thread_id, K* keys_input, K* keys_tmp, V* values_input, V* values_tmp, int num_elements_to_sort
)
{
    KeyToUint key_converter;
    ...
    for (int pass_id = 0; pass_id < num_passes; ++pass_id) {
        ...
        for (int j = thread_id; j < num_inner_loops * num_threads; j += num_threads) {
            int digit = j < num_elements_to_sort
                ? (int)((key_converter.convert(keys_input[j]) >> shift) & lowest_bits_mask)
                : num_scan_buckets;
        ...
        // The problem is that we either store local_offset_per_thread for every element array (potentially many) or we
        // recompute it again
        for (int j = thread_id; j < num_inner_loops * num_threads; j += num_threads) {
            int digit = j < num_elements_to_sort
                ? (int)((key_converter.convert(keys_input[j]) >> shift) & lowest_bits_mask)
            ...
            for (int b = 0; b < num_scan_buckets; b++) {
                ...
                if (contributes)
                    local_offset_per_thread = sum_per_warp
                        - 1;  //-1 because of inclusive scan and local_offset_per_thread needs exclusive scan
            }
...
inline CUDA_CALLABLE void radix_sort_thread_block(
    int thread_id, int* keys_input, int* keys_tmp, V* values_input, V* values_tmp, int num_elements_to_sort
)
{
    radix_sort_thread_block_core<num_warps, num_threads, int, V, IntKeyToUint>(
        thread_id, keys_input, keys_tmp, values_input, values_tmp, num_elements_to_sort
    );
}
...
template <int num_warps, int num_threads, typename V>
inline CUDA_CALLABLE void radix_sort_thread_block(
    int thread_id, float* keys_input, float* keys_tmp, V* values_input, V* values_tmp, int num_elements_to_sort
)
{
...
template <typename TileK, typename TileV> CUDA_CALLABLE_DEVICE void tile_sort(TileK& t, TileV& t2)
{
    ...
    if constexpr (num_elements_to_sort <= BITONIC_SORT_THRESHOLD || sizeof(T) > 4) {
        if constexpr (is_power_of_two(num_elements_to_sort))
            bitonic_sort_thread_block_direct<num_elements_to_sort, V>(
                WP_TILE_THREAD_IDX, keys, values, num_elements_to_sort
            );
        else
            bitonic_sort_thread_block_shared_mem<num_elements_to_sort, V>(
                WP_TILE_THREAD_IDX, keys, values, num_elements_to_sort
            );
    } else {
        ...
        radix_sort_thread_block<warp_count, WP_TILE_BLOCK_DIM, V>(
            WP_TILE_THREAD_IDX, keys, keys_tmp, values, values_tmp, num_elements_to_sort
        );
    }
...
CUDA_CALLABLE_DEVICE void tile_sort(TileK& t, TileV& t2, int start, int length)
{
    ...
    if (num_elements_to_sort <= BITONIC_SORT_THRESHOLD || sizeof(T) > 4) {
        if (is_power_of_two(num_elements_to_sort))
            bitonic_sort_thread_block_direct<max_elements_to_sort, V>(
                WP_TILE_THREAD_IDX, keys, values, num_elements_to_sort
            );
        else
            bitonic_sort_thread_block_shared_mem<max_elements_to_sort, V>(
                WP_TILE_THREAD_IDX, keys, values, num_elements_to_sort
            );
    } else {
...
template <typename K, typename V> void bitonic_sort_pairs_pow2_length_cpu(K WP_TILE_SHARED* keys, V WP_TILE_SHARED* values, int length)
{
...
void bitonic_sort_pairs_general_size_cpu(WP_TILE_ARENA_PARAM K WP_TILE_SHARED* keys, V WP_TILE_SHARED* values, int length)
{
...
template <typename V, int max_size> void bitonic_sort_pairs_general_size_cpu(WP_TILE_ARENA_PARAM unsigned int WP_TILE_SHARED* keys, V WP_TILE_SHARED* values, int length)
{
    bitonic_sort_pairs_general_size_cpu<unsigned int, V, max_size, UintKeyToUint>(WP_TILE_ARENA_ARG keys, values, length);
}

template <typename V, int max_size> void bitonic_sort_pairs_general_size_cpu(WP_TILE_ARENA_PARAM int WP_TILE_SHARED* keys, V WP_TILE_SHARED* values, int length)
{
    bitonic_sort_pairs_general_size_cpu<int, V, max_size, IntKeyToUint>(WP_TILE_ARENA_ARG keys, values, length);
}

template <typename V, int max_size> void bitonic_sort_pairs_general_size_cpu(WP_TILE_ARENA_PARAM float WP_TILE_SHARED* keys, V WP_TILE_SHARED* values, int length)
{
    bitonic_sort_pairs_general_size_cpu<float, V, max_size, FloatKeyToUint>(WP_TILE_ARENA_ARG keys, values, length);
}

template <typename V, int max_size> void bitonic_sort_pairs_general_size_cpu(WP_TILE_ARENA_PARAM int64_t WP_TILE_SHARED* keys, V WP_TILE_SHARED* values, int length)
{
    bitonic_sort_pairs_general_size_cpu<int64_t, V, max_size, Int64KeyToUint>(WP_TILE_ARENA_ARG keys, values, length);
}

template <typename V, int max_size> void bitonic_sort_pairs_general_size_cpu(WP_TILE_ARENA_PARAM uint64_t WP_TILE_SHARED* keys, V WP_TILE_SHARED* values, int length)
{
...
template <typename K, typename V, typename KeyToUint>
void radix_sort_pairs_cpu_core(K WP_TILE_SHARED* keys, K WP_TILE_SHARED* aux_keys, V WP_TILE_SHARED* values, V WP_TILE_SHARED* aux_values, int n)
{
    KeyToUint converter;
...
template <typename V>
inline void
radix_sort_pairs_cpu(int WP_THREAD* keys_input, int WP_THREAD* keys_aux, V WP_TILE_SHARED* values_input, V WP_TILE_SHARED* values_aux, int num_elements_to_sort)
{
...
inline void radix_sort_pairs_cpu(
    unsigned int WP_TILE_SHARED* keys_input, unsigned int WP_TILE_SHARED* keys_aux, V WP_TILE_SHARED* values_input, V WP_TILE_SHARED* values_aux, int num_elements_to_sort
)
{
...
template <typename V>
inline void
radix_sort_pairs_cpu(float WP_TILE_SHARED* keys_input, float WP_TILE_SHARED* keys_aux, V WP_TILE_SHARED* values_input, V WP_TILE_SHARED* values_aux, int num_elements_to_sort)
{
    ...
    if (WP_TILE_THREAD_IDX == first_active_lane) {
        if constexpr (num_elements_to_sort <= BITONIC_SORT_THRESHOLD || sizeof(T) > 4) {
            if constexpr (is_power_of_two(num_elements_to_sort))
                ...
                bitonic_sort_pairs_general_size_cpu<V, num_elements_to_sort>(WP_TILE_ARENA_ARG keys, values, num_elements_to_sort);
...
template <typename TileK, typename TileV> void tile_sort(WP_TILE_ARENA_PARAM TileK WP_THREAD& t, TileV WP_THREAD& t2, int start, int length)
{
    using T = typename TileK::Type;
    using V = typename TileV::Type;
    ...
    if (WP_TILE_THREAD_IDX == first_active_lane) {
        if (num_elements_to_sort <= BITONIC_SORT_THRESHOLD || sizeof(T) > 4) {
            if (is_power_of_two(num_elements_to_sort))
                ...
                bitonic_sort_pairs_general_size_cpu<V, max_elements_to_sort>(WP_TILE_ARENA_ARG keys, values, num_elements_to_sort);
...
template <typename TileK, typename TileV> inline void adj_tile_sort(TileK WP_THREAD& t, TileV WP_THREAD& t2, TileK WP_THREAD& adj_t1, TileV WP_THREAD& adj_t2)
{
...
template <typename TileK, typename TileV>
inline void
adj_tile_sort(TileK WP_THREAD& t, TileV WP_THREAD& t2, int start, int length, TileK WP_THREAD& adj_t1, TileV WP_THREAD& adj_t2, int adj_start, int adj_length)
{
