...
mujoco-warp = { git = "https://github.com/DavidDobas/mujoco_warp", branch = "metal" }
```
...
The backend is developed and tested in a fork of Warp, https://github.com/DavidDobas/warp
(branch `daviddobas/metal-backend`). This repository is generated from it:

```
python tools/generate.py <fork checkout> --warp-version <warp-lang version>
```

copies the changed Python modules into `src/warp_metal/_overlay`, the native headers into
`src/warp_metal/native`, the core library built in the fork into `src/warp_metal/bin`, and records
the fork commit in `_generated.py`. The LLVM helper library used for CPU kernels comes from the
stock `warp-lang` package. Everything under those three directories is generated; edit the fork.
